...
def step_impl(context):
    commit_store = create_async_commit_store(connection_string=context.db, storage_type=context.storage_type,
...
def step_impl(context):
    commit_store = create_commit_store(connection_string=context.db, storage_type=context.storage_type,
                                       topic_map=context.topic_map)
...
async def step_impl(context):
    saga = await context.repository.get(TestSaga, 'test')
    saga.transition(
        TestEvent(value=1, source='test', version=0, timestamp=datetime.datetime.now(datetime.timezone.utc)))
    context.saga = saga
