...
def step_impl(context):
    ...
    context.store = create_async_commit_store(context.db, context.storage_type, context.topic_map, conflict_detector)


@when("I have an commit store with a conflict detector")
def step_impl(context):
    conflict_detector = ConflictDetector([TestEventConflictDelegate()])
    context.store = create_commit_store(context.db, context.storage_type, context.topic_map, conflict_detector)
