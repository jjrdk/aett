...
def step_impl(context):
    assert hasattr(context, 'exception'), "Expected exception to be thrown, but none was"
    assert isinstance(context.exception,
                      ConflictingCommitException), f"Expected exception to be of type ConflictingCommitException, but was {type(context.exception)}"
...
def step_impl(context):
    assert hasattr(context, 'exception'), "Expected exception to be thrown, but none was"
    assert isinstance(context.exception,
                      NonConflictingCommitException), f"Expected exception to be of type NonConflictingCommitException, but was {type(context.exception)}"
