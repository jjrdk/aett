...
def step_impl(context):
    ...
    context.store = CommitStore(conflict_detector=conflict_detector, region='localhost', topic_map=topic_map)
    ...
    try:
        context.store.commit(
            Commit(tenant_id=context.tenant_id,
                   ...
                   headers={},
                   events=[EventMessage(
                       body=TestEvent(source='test', timestamp=datetime.datetime.now(), version=1, value=0))],
                   checkpoint_token=0))
